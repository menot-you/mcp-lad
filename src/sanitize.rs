...
pub fn is_safe_url(url: &str) -> bool {
    ...
    // Protocol-relative references (`//host/path`) are not safe relative
    // paths: browsers can treat them as network navigations using the
    // current scheme, bypassing the host checks below.
...
mod tests {
    ...
    #[test]
    fn blocks_protocol_relative_private_host() {
        assert!(!is_safe_url("//169.254.169.254/latest/meta-data/"));
        assert!(!is_safe_url("//127.0.0.1:3000/admin"));
    }

    #[test]
    fn blocks_backslash_protocol_relative_private_host() {
        assert!(!is_safe_url("\\\\169.254.169.254\\latest\\meta-data"));
        assert!(!is_safe_url("\\\\localhost\\admin"));
    }
