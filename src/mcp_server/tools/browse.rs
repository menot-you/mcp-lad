...
impl LadServer {
    ...
    pub(crate) async fn tool_lad_browse(
        ...
        let log_goal = if let Some(idx) = p.goal.to_lowercase().find("password") {
            // MSRV-gap: `ceil_char_boundary` is stable since 1.91; workspace metadata
            // declares rust-version 1.89, but the toolchain (rust-toolchain.toml) and all
            // CI jobs pin 1.94.1 — nothing builds at 1.89, so this is benign in practice.
